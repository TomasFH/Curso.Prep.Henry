...
function numeroMasGrande(numeros) {
  ...
  // Tu código:
  var bnumero = numeros[0]
  for (let i = 1; i < numeros.length; i++) {
      if (bnumero <= numeros[i]) {
        bnumero = numeros[i];        
      }
  }
  return bnumero
...
function breakStatement(numero) {
  ...
  for (var i = 0; i < 10; i++) {
    ...
    if( respuesta ===  i) break;
    else {
        ValorAumentado.push(respuesta);
    }
  }
    if ( i < 10){
